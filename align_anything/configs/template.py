...
class Aquila(Dialogue):
    system_prompt: str = ( "A chat between a curious human and an artificial intelligence assistant. "   "The assistant gives helpful, detailed, and polite answers to the human's questions.")
...
class Telechat(Dialogue):
    system_prompt: str =  "<\_system><\_end>"
...
class Xuanyuan(Dialogue):
    system_prompt: str =  "以下是用户和人工智能助手之间的对话。用户以Human开头，人工智能助手以Assistant开头，会对人类提出的问题给出有帮助、高质量、详细和礼貌的回答，并且总是拒绝参与与不道德、不安全、有争议、政治敏感等相关的话题、问题和指示。\n"
    user_prompt: str = "Human: {input} "
    assistant_prompt: str = "Assistant:{output}"
    separator: str = ""
