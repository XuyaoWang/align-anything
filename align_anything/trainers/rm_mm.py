...
from align_anything.datasets.preference_mm import MMPreferenceBatch, MMPreferenceDataset
...
from align_anything.models.any_model import AnyModelConfig
...
class RMTrainer:
    ...
    def init_datasets(self) -> None:
        ...
        train_dataset = MMPreferenceDataset(
        ...
        if self.cfgs.data_cfgs.eval_datasets:
            eval_dataset = MMPreferenceDataset(
    ...
    def loss(
        self,
        batch: MMPreferenceBatch,
    ) -> dict[str, torch.Tensor]:
        """Loss function for the reward model."""
        (
            better_pixel_values,  # size = (B, C, H, W) or (B, T, C, H, W) or (B, N, C, M, F)
            worse_pixel_values,  # size = (B, C, H, W) or (B, T, C, H, W) or (B, N, C, M, F)
        ) = batch[
            'pixel_values'
        ].chunk(chunks=2, dim=0)
        assert better_pixel_values.size(0) == worse_pixel_values.size(0), 'batch size mismatch!'
...
def main():
    ...
    dict_cfgs, ds_cfgs = read_cfgs(mode='train', task='rm_mm')
